...
def mx_doit_rester_doux(mx):
    """True pour les MX qu'on ne doit pas accelerer agressivement."""
    mx = (mx or "").lower()
    return any(mot in mx for mot in MX_DOUX)


def plancher_delai_mx(mx, confiance):
    """Decide le delai minimum autorise selon la confiance du MX.

    Version SPEED : les MX standards peuvent descendre bas, mais OVH garde
    un rythme doux pour eviter les blocages et les rate-limits.
    """
    mx = mx.lower()
    confiance = normaliser_confiance(confiance)

    if mx_doit_rester_doux(mx):
